...
Mirrors the authoritative TS ``PlotSpec``/``DataSpec``/``PlotNode`` in
``packages/spec/src/spec.ts`` (from which ``schema/cairn-plot-spec.schema.json``
is generated by ``ui/scripts/gen-plot-spec-schema.mjs``); kept honest by
``tests/test_spec_conformance.py`` field-for-field against that committed schema.
Construction == validation, like ``CardSpec``.

The single accepted descriptor shape is the recursive tree ``PlotSpec``
...
class NpzDataSpec(_Strict):
    ...
    kind: Literal["npz"]
    hash: Optional[str]
    # Enum-locked to the four 3D object types (mirrors the TS
    # `objectType: "pointcloud" | "mesh" | "volume" | "boxes3d"` and the schema's
    # `NpzDataSpec.objectType` enum); the conformance test guards the parity.
    objectType: Literal["pointcloud", "mesh", "volume", "boxes3d"]
    meta: dict[str, Any]
...
# `PlotNode`/`PlotSpec` in ``packages/spec/src/spec.ts`` (from which
# ``schema/cairn-plot-spec.schema.json`` is generated). Discriminated on
...
class PlotSpec(_Strict):
    ...
    (== TS ``PlotSpec``). ``mode``/``endpoint`` bind the whole tree to one
