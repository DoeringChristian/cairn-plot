...
colormap / HDR tone-map / public compare-kernel sets without failing a guard:
  ...
  - `_COMPARE_OPERATION_MODES`    ↔ contract `comparisonOperationPublicNames`
...
def test_compare_kernel_modes_mapping_matches_contract() -> None:
    # Audit M6: guard the public-mode → `operation` mapping VALUES, not just the
    ...
    # same JSON transitively pins the emitted `operation` to a real kernel id.
    assert _COMPARE_OPERATION_MODES == CONTRACT["comparisonOperationModes"]
