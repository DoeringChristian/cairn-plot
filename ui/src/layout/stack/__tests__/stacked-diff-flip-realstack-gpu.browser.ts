  ...
  try {
    ...
    const runAdoption = async (
      ...
      const settled = await waitFor(() => {
        ...
        const img = log.some((r) => r.mode === "image" && !r.imageOperation && !r.hasSrcB);
        const diff = log.some((r) => r.mode === "cached-diff" || !!r.imageOperation);
        return img && diff;
      }, 12000, 30);
      ...
      const imagePresents = log.filter((r) => r.mode === "image" && !r.imageOperation && !r.hasSrcB);
    ...
    const observeAfter = async (
      ...
      const log = getPaneRenderLog();
      stopPaneRenderLog();
      const img = log.filter((r) => r.mode === "image" && !r.imageOperation && !r.hasSrcB);
