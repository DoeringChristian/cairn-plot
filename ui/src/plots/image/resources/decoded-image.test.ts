...
test("a total failure resolves null EVERY time it is asked (no negative cache)", async () => {
  const url = nextUrl();
  ...
  // No negative window: the very next ask retries.
  assert.equal(await decodedImage(url, { deps: h.deps }), null);
  assert.equal(h.calls.fetchBlob, 2, "a failed decode is retried, not cached");
  assert.equal(h.calls.decodeElement, 2);

  // And a later attempt can succeed with no clock advance at all.
  const ok = harness();
  ok.clock.value = h.clock.value;
  const entry = await decodedImage(url, { deps: ok.deps });
  assert.ok(entry, "the retry succeeded immediately");
