...
function expectedRGB(px: number[], enc: DisplayOperation, gamma: number | undefined, hdrOut: boolean): RgbTriple {
  ...
  const toned = evaluateDisplayOperation(enc, exposed, 3, { ...DEFAULT_ENCODE_PARAMS, peak: HARNESS_PEAK });
  if (hdrOut) {
...
async function runEncodingCase(device: Device, enc: DisplayOperation, hdrOut: boolean): Promise<boolean> {
  const pixels = enc.category === "remap" ? SIGNED_PIXELS : GRADIENT_PIXELS;
  ...
  report(ok, `[${enc.id}] (${enc.category}${hdrOut ? ", hdrOut" : ""}) GPU applyOperator === cpu twin`);
  return ok;
}
...
async function runLutCase(device: Device, enc: DisplayOperation): Promise<boolean> {
  const lut = (enc.implementation.kind === "lut" ? enc.implementation.table : colormapFloatLUT(enc.id as ColormapName));
  const params: ImageParams = {
    exposureEV: EV,
    displayOperationId: enc.id,
    isScalar: true,
  ...
  for (let i = 0; i < SCALAR_PIXELS.length; i++) {
    ...
    const exp = evaluateDisplayOperation(enc, [applyExposure(SCALAR_PIXELS[i]![0]!, EV)], 1, {
      ...DEFAULT_ENCODE_PARAMS,
    });
    for (let c = 0; c < 3; c++) {
      const eb = byteOf(exp[c]!);
...
async function runLutNormCase(
  ...
  let ok = true;
  for (let i = 0; i < SCALAR_PIXELS.length; i++) {
    const exp = evaluateDisplayOperation(enc, [applyExposure(SCALAR_PIXELS[i]![0]!, EV)], 1, encParams);
    for (let c = 0; c < 3; c++) {
      const eb = byteOf(exp[c]!);
      const ab = out[i * 4 + c]!;
      if (Math.abs(ab - eb) > 1) {
 ...
 * (`evaluateDisplayOperation(enc, rgb, 3, {reduce})`). Nearest filter + EV 0 → byte-exact (within 1/255).
 */
async function runLutReduceCase(
  ...
  for (let i = 0; i < MULTI_PIXELS.length; i++) {
    ...
    const exp = evaluateDisplayOperation(enc, [px[0]!, px[1]!, px[2]!], 3, { ...DEFAULT_ENCODE_PARAMS, reduce: mode });
    for (let c = 0; c < 3; c++) {
      const eb = byteOf(exp[c]!);
      const ab = out[i * 4 + c]!;
      if (Math.abs(ab - eb) > 1) {
...
async function runTurboCase(device: Device, enc: DisplayOperation): Promise<boolean> {
  ...
  for (let i = 0; i < TURBO_PIXELS.length; i++) {
    const exp = evaluateDisplayOperation(enc, [applyExposure(TURBO_PIXELS[i]![0]!, EV)], 1, { ...DEFAULT_ENCODE_PARAMS });
    for (let c = 0; c < 3; c++) {
      const eb = byteOf(exp[c]!);
      const ab = out[i * 4 + c]!;
      if (Math.abs(ab - eb) > 1) {
...
async function runAnalyticCase(device: Device, enc: DisplayOperation): Promise<boolean> {
  ...
  for (const hdrOut of [false, true]) {
    ...
    for (let i = 0; i < ANALYTIC_PIXELS.length; i++) {
      ...
      const lin = evaluateDisplayOperation(enc, [applyExposure(ANALYTIC_PIXELS[i]![0]!, EV)], 1, { ...DEFAULT_ENCODE_PARAMS });
      if (hdrOut) {
        if (!(out instanceof Float32Array)) {
...
async function main(): Promise<void> {
  try {
    ...
    for (const enc of encodings) {
      ...
      if (enc.implementation.kind === "analytic") {
