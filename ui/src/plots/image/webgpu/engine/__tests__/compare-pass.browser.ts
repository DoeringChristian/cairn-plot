...
function processSide(px: number[], params: CompareParams): RgbTriple {
  ...
  const toned = evaluateDisplayOperation(opEnc, exposed, 3, DEFAULT_ENCODE_PARAMS);
...
function expectedDiffPixel(pxRef: number[], pxFg: number[], operationId: string): RgbTriple {
  const range = displayRangeForOperation(getImageOperation(operationId)!.output.domain);
  const out: number[] = [];
  for (let c = 0; c < 3; c++) {
    const a = pxRef[c]!;
    const b = pxFg[c]!;
