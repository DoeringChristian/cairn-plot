...
test("seriesWeightsFor translates single/luma/mean specs into component weights", async () => {
  const { seriesWeightsFor } = await import("../../../plots/image/components/image-histogram.ts");
...
test("computeDepthHistogramCpu bins alpha over the finite-z symlog range", async () => {
  const { computeDepthHistogramCpu } = await import("../../../plots/image/components/image-histogram.ts");
