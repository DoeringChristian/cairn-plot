...
import { __resetSettingsChannelsForTest } from "../../lib/cairn-plot/settings/viewport-settings.ts";
import { __resetSettingsPeersForTest, peekGroupSettings } from "../../lib/cairn-plot/settings/settings-peers.ts";
...
test("non-camera patches ride inert (mixed-kind group): absorbed, camera untouched", async () => {
  const g = freshGroup();
  const { publishSettingsPatch } = await import("../../lib/cairn-plot/settings/viewport-settings.ts");
  const seen: CameraState[] = [];
