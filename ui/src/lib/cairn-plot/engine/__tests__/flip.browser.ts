/**
 * FLIP: GPU multi-pass kernel vs CPU reference harness (WebGPU, driven via
 ...
 *
 * GPU stores YCxCz/Lab intermediates in rgba16float (spec-mandated), so the
 * GPU↔CPU tolerance is looser than the CPU↔official one — the perceptual FLIP
 * map still agrees to a few 1e-2 per pixel, well below what matters visually.
 *
...
async function runFlipCase(device: Device, w: number, h: number, seed: number): Promise<boolean> {
  ...
  if (!(gpu instanceof Float32Array)) {
    report(false, `[flip ${w}x${h}] readback should be Float32Array`);
    return false;
  }
  ...
  let sum = 0;
  for (let i = 0; i < w * h; i++) {
    const g = gpu[i * 4]!; // flip written to all channels
